...
with open(os.path.join(DataFolder,'TestCase.txt'),'r') as f:
    ...
    for item in TestCases:
        ...
        if os.path.exists(os.path.join(DataFolder_case,'except.csv')) and os.path.exists(os.path.join(DataFolder_case,'alarm_csv.csv')):
            ...
            df_videopath['video path'] = df_videopath['video name'].apply(lambda x:os.path.join(DataFolder_case,x))
            print(df_videopath)
...
#df_allCasesResult = df_allCasesResult.drop(['time'],axis=1)
    ...
    #os.remove(os.path.join(DataFolder,'Result'))
...
shutil.copy(os.path.join(DataFolder,'version.txt'),os.path.join(path_result,'version.txt'))